...
from SpriteSomethingPy import widgetlib
from SpriteSomethingPy import romhandler
from SpriteSomethingPy import common
from SpriteSomethingPy import gui_common #TODO: Should not use GUI stuff in game class, need to move this elsewhere
...
def get_game_class_of_type(game_name):
	#dynamic import
	game_module = importlib.import_module(f"SpriteSomethingPy.{game_name}.game")
...
class GameParent():
	...
	def load_plugins(self):
		try:
			plugins_module = importlib.import_module(f"SpriteSomethingPy.{self.internal_name}.plugins")
	...
	def make_sprite_by_number(self, sprite_number, sprite_filename):
		...
		if str(sprite_number) in manifest:
			...
			source_subpath = f"SpriteSomethingPy.{self.internal_name}.{folder_name}"
			...
			try:
				...
				animationlib = importlib.import_module(f"SpriteSomethingPy.animationlib")
	...
	def get_rom_from_filename(self, filename):
		#dynamic import
		rom_module = importlib.import_module(f"SpriteSomethingPy.{self.internal_name}.rom")
